...
tag name. `lib/turnstile.ts` keeps the old helper's contract and now passes `remoteip` and logs
Cloudflare's `error-codes`. The widget is also torn down on unmount (the hCaptcha version leaked
one per mount as users switched login/forgot tabs), and error/timeout callbacks clear the token.

This repo specifically: `app/contact`, `app/login`, `app/signup`, `app/test-booking/login`,
`app/test-booking/signup`, both API routes, `app/cookies` prose, CSP in `next.config.ts`
(`challenges.cloudflare.com`; **Stripe CSP entries untouched**), all 6 locales, and the locale key
