...
// ── hreflang / localized URLs ──────────────────────────────────────────────────
// Each language version of a guide (and of the guides index) is a distinct,
// self-canonical URL. hreflang tags cross-link the set so Google serves the right
// language per user and doesn't treat them as duplicates.
const SITE = "https://www.camel-global.com";

/** Our lang codes already match hreflang codes; kept explicit for clarity. */
export const HREFLANG: Record<GuideLang, string> = {
  en: "en", es: "es", fr: "fr", it: "it", pt: "pt", de: "de",
};

/** Languages that actually have a post for this slug (drives hreflang). */
export function guideLangsForSlug(slug: string): GuideLang[] {
  return GUIDE_LANGS.filter((l) => listGuides(l).some((g) => g.slug === slug));
}

/** hreflang alternates for a single guide post, across the languages it exists in. */
export function guidePostAlternates(slug: string): Record<string, string> {
  const out: Record<string, string> = {};
  for (const l of guideLangsForSlug(slug)) out[HREFLANG[l]] = `${SITE}/${l}/guides/${slug}`;
  out["x-default"] = `${SITE}/${PRIMARY_GUIDE_LANG}/guides/${slug}`;
  return out;
}

/** hreflang alternates for the guides index, across languages that have content. */
export function guideIndexAlternates(): Record<string, string> {
  const out: Record<string, string> = {};
  for (const l of getGuideLangs()) out[HREFLANG[l]] = `${SITE}/${l}/guides`;
  out["x-default"] = `${SITE}/${PRIMARY_GUIDE_LANG}/guides`;
  return out;
}

/** Country codes that have posts IN a specific language (lang-scoped sidebar). */
export function getGuideCountriesForLang(lang: string): GuideCountry[] {
  const counts = new Map<string, number>();
  for (const p of listGuides(lang)) {
    const c = (p.country || "").toUpperCase();
    if (!c) continue;
    counts.set(c, (counts.get(c) || 0) + 1);
  }
...
/** Posts for a country IN a specific language, newest first. */
export function guidesByCountryForLang(lang: string, country: string): GuideMeta[] {
  const code = (country || "").toUpperCase();
  return listGuides(lang).filter((p) => (p.country || "").toUpperCase() === code);
}
